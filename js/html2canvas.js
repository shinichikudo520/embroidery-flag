...
var screenshot = {
	init:function(id,callBack){
		//screenshot.setListener();
		//alert('init');
		screenshot.html2Canvas(id,callBack);
	},
	//设置监听事件
	...
	//绘制dom 元素，生成截图canvas
	html2Canvas: function (id,callBack) {
		var shareContent = document.querySelector("#"+id);// 需要绘制的部分的 (原生）dom 对象 ，注意容器的宽度不要使用百分比，使用固定宽度，避免缩放问题
		...
		var scaleBy = screenshot.getPixelRatio(context);  //获取像素密度的方法 (也可以采用自定义缩放比例)
